package io.github.xtrlumen.vbmeta.repository;
...
import static io.github.xtrlumen.vbmeta.lang.AttestationException.*;
...
import io.github.xtrlumen.vbmeta.keystore.AndroidKeyStore;
import io.github.xtrlumen.vbmeta.keystore.IAndroidKeyStore;
import io.github.xtrlumen.vbmeta.keystore.KeyStoreManager;
import io.github.xtrlumen.vbmeta.lang.AttestationException;
import io.github.xtrlumen.vbmeta.util.Resource;
...
public class AttestationRepository {
    ...
    public Resource<AttestationData> attest(boolean reset, boolean useAttestKey, boolean useStrongBox, boolean includeProps, boolean uniqueIdIncluded, int idFlags, boolean useSak) {
        currentCerts.clear();
        try {
            if (reset) keyStore.deleteAllEntry();
            doAttestation(useAttestKey, useStrongBox, includeProps, uniqueIdIncluded, idFlags, useSak);
            return Resource.success(AttestationData.parseCertificateChain(currentCerts));
        } catch (Exception e) {
            ...
            if (e instanceof AttestationException) {
                ...
                return Resource.error(new AttestationException(CODE_UNKNOWN, e), null);
            }
        }
    }
    ...
    public Resource<AttestationData> loadCerts(ParcelFileDescriptor pfd) {
        ...
        try {
            ...
            if (e instanceof AttestationException) {
                ...
                return Resource.error(new AttestationException(CODE_CANT_PARSE_CERT, e), null);
            } else {
                return Resource.error(new AttestationException(CODE_UNKNOWN, e), null);
            }
        }
    ...
    public Resource<RemoteProvisioningData> checkRkp(boolean useStrongBox) {
        ...
        try {
            ...
            if (e instanceof IllegalStateException) {
                return Resource.error(new AttestationException(CODE_RKP, e), null);
            } else {
                return Resource.error(new AttestationException(CODE_UNKNOWN, e), null);
            }
        }
